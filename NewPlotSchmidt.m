function handle_out = NewPlotSchmidt(handle, MODEL, SITE, t_p, x_p, y_p, z_p, n_vel_site, OPTIONS)
...
if ShowNet
  for i=1:length(lambda_grid)
    [x_grid,y_grid,z_grid] = DipAndAzimuth_to_xyz(pi*phi_grid/180, pi*lambda_grid(i)/180*ones(1, numel(phi_grid)), 'upper');
  ...
  clear x_grid y_grid z_grid X_s Y_s

  for i=1:length(phi_grid)
    [x_grid,y_grid,z_grid] = DipAndAzimuth_to_xyz(pi*phi_grid(i)/180*ones(1,numel(lambda_fine_grid)), pi*lambda_fine_grid/180, 'upper');
    [X_s,Y_s] = ProjectDown(x_grid, y_grid, z_grid);
    if Monochrome
      plot(X_s, Y_s, 'k')
    else  
...
if ~HAMBREY_DATA
  ...
  plot(X_s, Y_s, 'LineStyle', 'none', 'MarkerFaceColor', marker_face_color_default, 'MarkerEdgeColor', marker_face_color_default, ...
