function handle_out = Triptych(handle, MODEL, SITE, N_pts, x_p, y_p, z_p, n_vel_site, PlotTitle, OPTIONS, VERBOSITY)
...
NewPlotSchmidt([], MODEL, SITE, [], x_p, y_p, z_p, n_vel_site, OPTIONS);
ax4 = gca;
set(ax4,'Title',[])
...
if any(fig_pos_start-get(handle, 'position'))
  fprintf(1,'Triptych_2(1): Figure changes size for Site %s\n', char(SITE.Name_site))
end

if ~isfield(OPTIONS, 'Rosette') || isfield(OPTIONS, 'Rosette') && OPTIONS.Rosette
  g6 = subplot(3,3,6,'Parent',hp);
  axis off
  ...
  NewPlotRosettes([], MODEL, SITE, x_p, y_p, z_p, n_vel_site, OPTIONS);  
  ax6 = gca;
  set(ax6,'Title',[])
end
...
if HardCopy && ~isempty(handle)
  if isempty(dir_PLOT)
    dir_PLOT = pwd;
   dir_PLOT = fullfile(dir_PLOT, 'TriptychPlots', sprintf('CrackRule_#%02d', OPTIONS.CrackRule), sprintf('%s', OPTIONS.Polarity));    
  end  
